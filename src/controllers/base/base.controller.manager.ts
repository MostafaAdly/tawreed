...
export default class ControllersManager {
    ...
    private controllersDirectory: string = path.join(process.cwd(), 'src/controllers');
    ...
    private loadHandler = (parentDirectory: string, handlerName: string): Function | null => {
        for (let file of Helpers.searchFilesWithFunction(parentDirectory, handlerName).filter(file => file.endsWith(this.handlerSuffix))) {
            try {
                const handler = new (require(file).default)();
