        ...
        (async () => {
            try {
                const response = (await axios.post(getAPIURL('/posts/offers'), {
                    offersIDs, status: OfferStatus.Shipped, relations: ['offerResponse']
                })).data;
                if (response?.data) {
    ...
    return (
        <ClientLayout user={user}>
            <div className="flex flex-col mb-5">
                <h1 className="text-3xl font-bold text-gray-700 dark:text-gray-200">العمليات المكتملة</h1>
                <p className="text-gray-500 dark:text-gray-400">هنا يمكنك مشاهدة عملياتك المكتملة</p>
            </div>
