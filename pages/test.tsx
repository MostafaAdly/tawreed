...
import { getAPIURL } from "public/assets/utils/helpers";
...
export default () => {
  ...
  useEffect(() => {
    ...
    ; (async () => {
      try {
        const response = (await axios.get(getAPIURL('/posts/offers?id=1'))).data;
        if (response?.data) setOffer(response.data[0]);
      } catch (error) {
        console.error(error);
      }
    })();
